...
\tIf you don't have knownsites, please use --no_BaseRecalibrator option.")
    ...
    if mode == "align":
        ...
        # 如果是大基因组
        if args.large_genome is True: args.hisat2 = HISAT2L
        run_sr_align(sr_aligner=args.sr_aligner, align_idx=args.align_idx, large_gn=args.large_genome,
                     seq_1=vars(args)["1"], seq_2=vars(args)["2"], seq_u=args.U,
                     seq_sra=args.sra, ref_gtf=args.ref_gtf,
                     hisat2_opts=args.hisat2_opts, hisat2=args.hisat2, hisat2_sps=args.hisat2_sps,
        ...
        run_lr_fusion(long_fusion_caller=args.long_fusion_caller,
                      ...
                      genome_bowtie2_idx=args.genome_bowtie2_idx,
                      transcriptome_bowtie2_idx=args.transcriptome_bowtie2_idx,
                      read_length=args.read_length,
        ...
        if (vars(args)["1"] == "" and vars(args)["2"] == "") and (args.U == ""):
            parser.print_help()
            logger.error(
                "In pipeline mode, only one input short-read type is possible: paired-end (--1 and --2) or unpaired (--U)")
            return os.EX_USAGE
        ...
        if do_short:
            for si, sample in enumerate(samples):
                ...
                for ri, replicate in enumerate(sample):
                    ...
                    if "align" not in args.exclude:
                        ...
                        logger.info("******************************************************************************")
                        alignments_bam[si][replicate], junctions_tab[si][replicate], junctions_bed[si][
                            replicate] = run_sr_align(sr_aligner=args.sr_aligner,
                    ...
                    if "reconstruct" not in args.exclude:
                        ...
                        logger.info("******************************************************************************")
                        transcripts[si][replicate], abundances[si][replicate] = run_reconstruct(
                    ...
                    if "quantify" not in args.exclude:
                        ...
                        logger.info("******************************************************************************")
                        quant[si][replicate] = run_quantify(quantifier=args.quantifier,
                                                            quantifier_idx=args.quantifier_idx,
                                                            seq_1=input_sr["1"][replicate],
                                                            seq_2=input_sr["2"][replicate],
                                                            seq_u=input_sr["U"][replicate],
                                                            salmon_k=args.salmon_k, libtype=args.libtype,
                                                            salmon_smem_opts=args.salmon_smem_opts, salmon=args.salmon,
                                                            start=0, sample=replicate, nthreads=args.threads,
                                                            ...
                                                            timeout=args.timeout, ignore_exceptions=True)
                    else:
                        logger.info("******************************************************************************")
                    ...
                    if "denovo" not in args.exclude:
                        ...
                        transcripts_dnv[si][replicate] = run_dnv_assemebly(assembler=args.assembler,
                                                                           ...
                                                                           oases=args.oases, velvetg=args.velvetg,
                                                                           velveth=args.velveth,
                                                                           oases_opts=args.oases_opts,
                                                                           velvetg_opts=args.velvetg_opts,
                                                                           velveth_opts=args.velveth_opts,
                                                                           start=0, sample=replicate,
            ...
            if "diff" not in args.exclude:
                logger.info("******************************************************************************")
                logger.info(
                    "Running differential analysis step (based on alignment-free quantification results) using %s for %s" % (
                    args.difftool, samples))
                logger.info("******************************************************************************")
                diff_af = run_diff(difftool=args.difftool,
                                   quant_files=[",".join([quant[si][replicate] for replicate in sample]) for si, sample
                ...
                logger.info("******************************************************************************")
                logger.info("Running differential analysis step (based on alignment results) using %s for %s" % (
                args.difftool, samples))
                logger.info("******************************************************************************")
                #         if use_tgtf
                diff_al = run_diff(difftool=args.difftool, quant_files="",
                                   alignments=[",".join([alignments_bam[si][replicate] for replicate in sample]) for
                                               si, sample in enumerate(samples)],
                                   transcripts_gtfs=[",".join([transcripts[si][replicate] for replicate in sample]) for
                                                     si, sample in enumerate(samples)],
                                   ref_gtf=args.ref_gtf,
                                   featureCounts_opts=args.featureCounts_opts, featureCounts=args.featureCounts,
                                   stringtie=args.stringtie, stringtie_merge_opts=args.stringtie_merge_opts,
                ...
                logger.info("******************************************************************************")
                logger.info(
                    "Excluding differential analysis step (based on alignment-free quantification results) using %s for %s" % (
                    args.difftool, samples))
                logger.info("******************************************************************************")
                diff_af = ""
                logger.info("******************************************************************************")
                logger.info("Excluding differential analysis step (based on alignment results) using %s for %s" % (
                args.difftool, samples))
                logger.info("******************************************************************************")
                diff_al = ""
                ...
                for ri, replicate in enumerate(sample):
                    if "variant" not in args.exclude:
                        ...
                        variants[si][replicate] = run_variant(variant_caller=args.variant_caller,
                                                              ...
                                                              IndelRealignment=args.IndelRealignment,
                                                              no_BaseRecalibrator=args.no_BaseRecalibrator,
                                                              AddOrReplaceReadGroups_opts=args.AddOrReplaceReadGroups_opts,
                                                              MarkDuplicates_opts=args.MarkDuplicates_opts,
                                                              SplitNCigarReads_opts=args.SplitNCigarReads_opts,
                    ...
                    if "editing" not in args.exclude:
                        logger.info("******************************************************************************")
                        logger.info(
                            "Running RNA editing calling step using %s for %s" % (args.editing_caller, replicate))
                        logger.info("******************************************************************************")
                        edits[si][replicate] = run_editing(editing_caller=args.editing_caller,
                                                           alignment=alignments_bam[si][replicate],
                                                           variant=variants[si][replicate],
                                                           strand_pos=args.strand_pos, genes_pos=args.genes_pos,
                                                           ref_genome=args.ref_genome, knownsites=args.knownsites,
                                                           giremi_dir=args.giremi_dir, htslib_dir=args.htslib_dir,
                                                           samtools=args.samtools, gatk=args.gatk,
                                                           java=args.java, giremi_opts=args.giremi_opts,
                        ...
                        logger.info("******************************************************************************")
                        logger.info(
                            "Excluding RNA editing calling step using %s for %s" % (args.editing_caller, replicate))
                        logger.info("******************************************************************************")
                        edits[si][replicate] = ""
                    ...
                    if "fusion" not in args.exclude:
                        ...
                        fusions[si][replicate] = run_fusion(fusion_caller=args.fusion_caller,
                                                            ...
                                                            input="%s,%s" % (input_sr["1"][replicate],
                                                                             input_sr["2"][
                                                                                 replicate]) if input_mode == "paired" else
                                                            input_sr["U"][replicate],
                                                            start=0,
                                                            fusioncatcher=args.fusioncatcher,
                                                            fusioncatcher_opts=args.fusioncatcher_opts,
                ...
                for si, sample in enumerate(samples):
                    ...
                    for ri, replicate in enumerate(sample):
                        ...
                        if "long_correct" not in args.exclude:
                            logger.info(
                                "******************************************************************************")
                            logger.info("Running long read error correction step using %s for %s" % (
                            args.long_corrector, replicate))
                            logger.info(
                                "******************************************************************************")
                            corrected[si][replicate] = run_lr_correct(long_corrector=args.long_corrector,
                                                                      kmer=args.kmer,
                                                                      solid=args.kmer, long=input_lr[replicate],
                                                                      short="%s,%s" % (input_sr["1"][replicate],
                                                                                       input_sr["2"][
                                                                                           replicate]) if input_mode == "paired" else
                                                                      input_sr["U"][replicate],
                                                                      lordec=args.lordec, lordec_opts=args.lordec_opts,
                            ...
                            logger.info(
                                "******************************************************************************")
                            logger.info("Excluding long read error correction step using %s for %s" % (
                            args.long_corrector, replicate))
                            logger.info(
                                "******************************************************************************")
                        ...
                        if "long_align" not in args.exclude:
                            logger.info(
                                "******************************************************************************")
                            logger.info("Running long read alignment step on corrected long-reads using %s for %s" % (
                            args.long_aligner, replicate))
                            logger.info(
                                "******************************************************************************")
                            alignments_lr[si][replicate] = run_lr_align(long_aligner=args.long_aligner,
                                                                        long=corrected[si][replicate] if corrected[si][
                                                                            replicate] else input_lr[replicate],
                                                                        genome_dir=args.star_genome_dir,
                                                                        ref_gtf=args.ref_gtf,
                                                                        starlong=args.starlong,
                                                                        starlong_opts=args.starlong_opts,
                                                                        sam2psl=args.sam2psl, samtools=args.samtools,
                                                                        start=0, sample=replicate,
                                                                        nthreads=args.threads,
                                                                        workdir=args.workdir, outdir=args.outdir,
                            ...
                            logger.info(
                                "******************************************************************************")
                            logger.info("Excluding long read alignment step on corrected long-reads using %s for %s" % (
                            args.long_aligner, replicate))
                            logger.info(
                                "******************************************************************************")
                        ...
                        if "long_reconstruct" not in args.exclude:
                            logger.info(
                                "******************************************************************************")
                            logger.info("Running long read transcriptome reconstruction step using %s for %s" % (
                            args.long_reconstructor, replicate))
                            logger.info(
                                "******************************************************************************")
                            transcripts_lr[si][replicate], abundances_lr[si][replicate] = run_lr_reconstruct(
                                long_reconstructor=args.long_reconstructor,
                                alignment=alignments_bam[si][replicate],
                            ...
                            logger.info(
                                "******************************************************************************")
                            logger.info("Excluding long read transcriptome reconstruction step using %s for %s" % (
                            args.long_reconstructor, replicate))
                            logger.info(
                                "******************************************************************************")
                        ...
                        if "long_fusion" not in args.exclude:
                            logger.info(
                                "******************************************************************************")
                            logger.info("Running long read fusion detection step using %s for %s" % (
                            args.long_reconstructor, replicate))
                            logger.info(
                                "******************************************************************************")
                            fusions_lr[si][replicate] = run_lr_fusion(long_fusion_caller=args.long_fusion_caller,
                                                                      ...
                                                                      long_fasta=corrected[si][replicate] if
                                                                      corrected[si][replicate] else input_lr[replicate],
                                                                      mode_number=args.mode_number,
                                                                      ref_genome=args.ref_genome,
                                                                      ref_all_gpd=args.ref_all_gpd,
                                                                      ref_gpd=args.ref_gpd,
                                                                      uniqueness_bedgraph=args.uniqueness_bedgraph,
                                                                      genome_bowtie2_idx=args.genome_bowtie2_idx,
                                                                      transcriptome_bowtie2_idx=args.transcriptome_bowtie2_idx,
                                                                      read_length=args.read_length,
                                                                      samtools=args.samtools, idpfusion=args.idpfusion,
                                                                      idpfusion_cfg=args.idpfusion_cfg,
                                                                      gmap=args.gmap, gmap_idx=args.gmap_idx,
                                                                      star_dir=args.star_dir,
                                                                      bowtie2_dir=args.bowtie2_dir,
                                                                      start=0, sample=replicate, nthreads=args.threads,
                                                                      workdir=args.workdir, outdir=args.outdir,
                                                                      timeout=args.timeout, ignore_exceptions=True)
                        else:
                ...
                for si, sample in enumerate(samples):
                    ...
                    for ri, replicate in enumerate(sample):
                        if "long_align" not in args.exclude:
                            logger.info(
                                "******************************************************************************")
                            logger.info("Running long read alignment step on original long-reads using %s for %s" % (
                            args.long_aligner, replicate))
                            logger.info(
                                "******************************************************************************")
                            alignments_lr[si][replicate] = run_lr_align(long_aligner=args.long_aligner,
                                                                        long=input_lr[replicate],
                                                                        genome_dir=args.star_genome_dir,
                                                                        ref_gtf=args.ref_gtf,
                                                                        starlong=args.starlong,
                                                                        starlong_opts=args.starlong_opts,
                            ...
                            logger.info(
                                "******************************************************************************")
                            logger.info("Excluding long read alignment step on original long-reads using %s for %s" % (
                            args.long_aligner, replicate))
                            logger.info(
                                "******************************************************************************")
                            alignments_lr[si][replicate] = ""
