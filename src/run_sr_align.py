...
def run_hisat2(align_idx=None, large_gn="",
               seq_1="", seq_2="", seq_u="",
               seq_sra="", ref_gtf="",
    ...
    if start <= step:
        logger.info("--------------------------STEP %s--------------------------" % step)
        logger.info('large genome {}'.format(large_gn))
        if large_gn is False:
        ...
        elif large_gn is True:
            # 大基因组暂时只支持 PE reads.
            seq_argument = "-1 <(zcat {}) -2 <(zcat {})".format(seq_1, seq_2)
            command = "%s %s  -x %s %s -S %s/alignments.sam --novel-splicesite-outfile %s/splicesites.tab" % (
                hisat2, hisat2_opts, align_idx, seq_argument, work_hisat2, work_hisat2)
            command = "bash -c \"%s\"" % command
            cmd = TimedExternalCmd(command, logger, raise_exception=True)
...
def run_sr_align(sr_aligner="HISAT2", align_idx=None, large_gn="",
                 seq_1="", seq_2="", seq_u="",
                 seq_sra="", ref_gtf="",
                 hisat2_opts="", hisat2=HISAT2, hisat2_sps=HISAT2_SPS,
                 samtools=SAMTOOLS,
    ...
    junctions_tab = ""
    junctions_bed = ""
    if sr_aligner.upper() == "HISAT2" or sr_aligner.upper() == "HISAT2L":
        # try:
        alignments_bam, junctions_tab, junctions_bed = run_hisat2(align_idx=align_idx, large_gn=large_gn,
                                                                  seq_1=seq_1, seq_2=seq_2, seq_u=seq_u,
                                                                  seq_sra=seq_sra, ref_gtf=ref_gtf,
                                                                  hisat2_opts=hisat2_opts, hisat2=hisat2,
